...
likeme <- function(skill1, job1, exp1, stype1, sk.ill, num1,clack, functional, systems, jobboard1){    
  ...
  if(stype1 == "eser"){
    ...
    #candidates <- candidates[,c(1,2,3,4,5,6,7,8,9)]
    #original <- original[,c(1,2,3,4,5,6,7,8,9)]
    candidates <- candidates[,c(1,2,3,4,5,6)]
    original <- original[,c(1,2,3,4,5,6)]
    candidates$TProfile <- ""
    original$TProfile <- ""
    if(sk.ill == "I have already entered the skills"){
      candidates$requirement <- candidates$Profile#Add the skills
...
server <- function(input, output, session) {
  ...
  #   selectInput("forecast.cr", "Select The Customer",
  #               if(input$forecast.ll =="ALL" & input$forecast.cc =="ALL"){
  #                 choices = c("ALL",unique(subset(demand.dump, tolower(demand.dump$country)==tolower(input$custloc))$Personal.SubArea))
  #               } 
  #               if(input$forecast.ll !="ALL" & input$forecast.cc =="ALL"){
  #                      choices = c("ALL",unique(subset(demand.dump, tolower(demand.dump$country)==tolower(input$custloc) & tolower(demand.dump$Joining.Level.2)==tolower(input$forecast.ll))$Personal.SubArea))
  #               }
  #                      if(input$forecast.ll =="ALL" & input$forecast.cc !="ALL"){      
  #                      choices = c("ALL",unique(subset(demand.dump, tolower(demand.dump$country)==tolower(input$custloc) & tolower(demand.dump$Customer)==tolower(input$forecast.cc))$Personal.SubArea))
  #                      }
  #                       else{ choices = c("ALL",unique(subset(demand.dump, tolower(demand.dump$country)==tolower(input$custloc) &  tolower(demand.dump$Joining.Level.2)==tolower(input$forecast.ll) & tolower(demand.dump$Customer)==tolower(input$forecast.cc))$Personal.SubArea))
  #               },
  #               #choices = c("ALL",unique(subset(demand.dump, tolower(demand.dump$country)%in%tolower(input$custloc))$Joining.Level.2)),
  #               selected = "option3"
  #   )
  ...
  output$forecast.creg <- renderUI({
    selectInput("forecast.cr",  "Select The Sub Area",
                if(input$forecast.ll !="ALL" & input$forecast.cc !="ALL"){
                  choices = c("ALL",unique(subset(demand.dump, tolower(demand.dump$country)==tolower(input$custloc) &  tolower(demand.dump$Joining.Level.2)==tolower(input$forecast.ll) & tolower(demand.dump$Customer)==tolower(input$forecast.cc))$Personal.SubArea))
                }else{ choices = c("ALL",unique(subset(demand.dump, tolower(demand.dump$country)==tolower(input$custloc))$Personal.SubArea))
               }
                ,
                #choices = c("ALL",unique(subset(demand.dump, tolower(demand.dump$country)%in%tolower(input$custloc))$Joining.Level.2)),
                selected = "option3"
  ...
  #               if(input$forecast.ll =="ALL" & input$forecast.cc =="ALL" & input$forecast.cr =="ALL"){
  #                 choices = unique(subset(demand.dump, tolower(demand.dump$country)==tolower(input$custloc))$Skill.Bucket)
  #                 #choices = unique(subset(demand.dump, tolower(demand.dump$country)%in%tolower(input$custloc))$Skill.Bucket),
  #               }else{if(input$forecast.ll =="ALL" & input$forecast.cc !="ALL" & input$forecast.cr !="ALL"){
  #                   choices = unique(subset(demand.dump, tolower(demand.dump$country)==tolower(input$custloc)&  tolower(demand.dump$Customer)==tolower(input$forecast.cc) & tolower(demand.dump$Personal.SubArea)==tolower(input$forecast.cr))$Skill.Bucket)
  #               }else{if(input$forecast.ll =="ALL" & input$forecast.cc !="ALL" & input$forecast.cr =="ALL"){
  #                         choices = unique(subset(demand.dump, tolower(demand.dump$country)==tolower(input$custloc)&  tolower(demand.dump$Customer)==tolower(input$forecast.cc))$Skill.Bucket)
  #               }else{if(input$forecast.ll =="ALL" & input$forecast.cc =="ALL" & input$forecast.cr !="ALL"){
  #                       choices = unique(subset(demand.dump, tolower(demand.dump$country)==tolower(input$custloc)&  tolower(demand.dump$Personal.SubArea)==tolower(input$forecast.cr))$Skill.Bucket)
  #               }else{if(input$forecast.ll !="ALL" & input$forecast.cc =="ALL" & input$forecast.cr =="ALL"){
  #                       choices = unique(subset(demand.dump, tolower(demand.dump$country)==tolower(input$custloc)& tolower(demand.dump$Joining.Level.2)==tolower(input$forecast.ll))$Skill.Bucket)
  #                       } else{choices = unique(subset(demand.dump, tolower(demand.dump$country)==tolower(input$custloc)& tolower(demand.dump$Joining.Level.2)==tolower(input$forecast.ll) & tolower(demand.dump$Customer)==tolower(input$forecast.cc) & tolower(demand.dump$Personal.SubArea)==tolower(input$forecast.cr))$Skill.Bucket)
  #                       }}}}},
  #              
  #                 #choices = unique(subset(demand.dump, tolower(demand.dump$country)%in%tolower(input$custloc) & tolower(demand.dump$Joining.Level.2)==tolower(input$forecast.ll))$Skill.Bucket)
  #                 
  #               selected = "option3"
  #   )
  # })
  ...
  output$forecast.skill <- renderUI({
    selectInput("forecast.ss", "Select Skill Bucket",
                if(input$forecast.ll !="ALL" & input$forecast.cc !="ALL" & input$forecast.cr !="ALL"){
                  choices = unique(subset(demand.dump, tolower(demand.dump$country)==tolower(input$custloc)&  tolower(demand.dump$Customer)==tolower(input$forecast.cc) & tolower(demand.dump$Joining.Level.2)==tolower(input$forecast.ll) & tolower(demand.dump$Personal.SubArea)==tolower(input$forecast.cr))$Skill.Bucket)
                 } else{ choices = unique(subset(demand.dump, tolower(demand.dump$country)==tolower(input$custloc))$Skill.Bucket)
                # }else{if(input$forecast.ll =="ALL" & input$forecast.cc !="ALL" & input$forecast.cr !="ALL"){
                #   choices = unique(subset(demand.dump, tolower(demand.dump$country)==tolower(input$custloc)&  tolower(demand.dump$Customer)==tolower(input$forecast.cc) & tolower(demand.dump$Personal.SubArea)==tolower(input$forecast.cr))$Skill.Bucket)
                # }else{if(input$forecast.ll =="ALL" & input$forecast.cc !="ALL" & input$forecast.cr =="ALL"){
                #   choices = unique(subset(demand.dump, tolower(demand.dump$country)==tolower(input$custloc)&  tolower(demand.dump$Customer)==tolower(input$forecast.cc))$Skill.Bucket)
  ...
  output$forecast.dcat <- renderUI({
    selectInput("forecast.dc", "Select Demand Category",
                if(input$forecast.ss=="All"){
                  choices = c("ALL",unique(subset(demand.dump, tolower(demand.dump$country)==tolower(input$custloc))$Requisition.Source))
                } else{choices = c("ALL",unique(subset(demand.dump, tolower(demand.dump$country)==tolower(input$custloc) & tolower(demand.dump$Skill.Bucket)==tolower(input$forecast.ss))$Requisition.Source))
                      },
                #choices = c("ALL",unique(subset(demand.dump, tolower(demand.dump$country)%in%tolower(input$custloc))$Joining.Level.2)),
                selected = "option3"
    )
  ...
  output$forecast.year <- renderUI({
    selectInput("forecast.yy", "Select Year",
                if(input$forecast.ll=="ALL"){
                  choices = unique(subset(demand.dump, tolower(demand.dump$country)==tolower(input$custloc))$year)
                  #choices = unique(subset(demand.dump, tolower(demand.dump$country)%in%tolower(input$custloc) & tolower(demand.dump$Skill.Bucket)==tolower(input$forecast.ss)) )$year
                }else{
                  choices = unique(subset(demand.dump, tolower(demand.dump$country)==tolower(input$custloc) & tolower(demand.dump$Joining.Level.2)==tolower(input$forecast.ll))$year)
                  #choices = unique(subset(demand.dump, tolower(demand.dump$country)%in%tolower(input$custloc) & tolower(demand.dump$Skill.Bucket)==tolower(input$forecast.ss) & tolower(demand.dump$Joining.Level.2)==tolower(input$forecast.ll))$year)  
                },
                selected = "option3"
    )
  })
  ...
  output$forecast.quarter <- renderUI({
    selectInput("forecast.qq", "Select Quarter",
                if(input$forecast.ll=="ALL"){
                  choices = unique(subset(demand.dump, tolower(demand.dump$country)==tolower(input$custloc) & demand.dump$year == input$forecast.yy)$quarter)
                  #choices = unique(subset(demand.dump, tolower(demand.dump$country)%in%tolower(input$custloc) & tolower(demand.dump$Skill.Bucket)==tolower(input$forecast.ss) & demand.dump$year == input$forecast.yy)$quarter) 
                }else{
                  choices = unique(subset(demand.dump, tolower(demand.dump$country)==tolower(input$custloc) & tolower(demand.dump$Joining.Level.2)==tolower(input$forecast.ll) & demand.dump$year == input$forecast.yy)$quarter)
                  #choices = unique(subset(demand.dump, tolower(demand.dump$country)%in%tolower(input$custloc) & tolower(demand.dump$Skill.Bucket)==tolower(input$forecast.ss) &tolower(demand.dump$Joining.Level.2)==tolower(input$forecast.ll) & demand.dump$year == input$forecast.yy)$quarter) 
                },
                selected = "option3"
    )
  })
