...
server <- function(input, output, session) {
  ...
  dd.1<-reactive({
    if(is.null(input$file1))
    {
      ...
      print(ncol(demand.dump))
      dd} else {
      write.csv(as.data.frame(data.append()[,138:ncol(data.append())]),"new_consolidated_skills")
        print("checking done")
  ...
  cons.1<-reactive({dta
    if(is.null(input$file1))
    {
      ...
      cons} else {
        ...
        write.csv(as.data.frame(data_cons),"new_consolidated")
        print("checking done")
  ...
  Append_demand<-function(){
    ...
    demand.dump<-smartbind(demand.dump,new.demand_load1)
     ...
     {
       cons1[,i]<-as.numeric(as.character(as.factor(demand.dump[,i])))
     }
