...
#     new.tokens <- tokens(new.demand$requirement, what = "word", remove_numbers = TRUE, remove_punct = TRUE)
#     new.tokens <- tokens_tolower(new.tokens)
#     new.tokens <- tokens_select(new.tokens, stopwords(), selection = "remove")
#     new.tokens <- tokens_wordstem(new.tokens, language = "english")
#     new.tokens.dfm <- dfm(new.tokens, tolower = FALSE)
#     new.tokens.matrix <- as.matrix(new.tokens.dfm)
# new.tokens.df <- data.frame(new.tokens.matrix)
# new.tokens.df$class.label <- new.demand$Skill.Bucket
...
master.length <- nrow(master)
new.length <- nrow(new.demand)
master1<-rbind(master,new.demand)
master1 <- master1[!duplicated(master1),]

#Remove observations from the data that do not have any requisition date.
#master1 <- master1[complete.cases(master1$Req.Date),]

master1$Segment <- factor(master1$Segment)
master1$Skill.Bucket <- factor(master1$Skill.Bucket)
master1$Requisition.Source <- factor(master1$Requisition.Source)
master1$requirement <- paste(master1$Primary.Skill.Area,master1$vAdditionalRemarks)
full.tokens <- tokens(master1$requirement, what = "word", remove_numbers = TRUE, remove_punct = TRUE)
full.tokens <- tokens_tolower(full.tokens)
full.tokens <- tokens_select(full.tokens, stopwords(), selection = "remove")
full.tokens <- tokens_wordstem(full.tokens, language = "english")
full.tokens.dfm <- dfm(full.tokens, tolower = FALSE)
full.tokens.matrix <- as.matrix(full.tokens.dfm)
full.tokens.df <- data.frame(full.tokens.matrix)
full.tokens.df$class.label <- master1$Skill.Bucket
skills.list <- skill.input
skills.list <- skill.input


if(nrow(master1) > nrow(master)){
...
test <- full.tokens.df[(master.length+1):nrow(full.tokens.df),]
      ...
      #Add the skills back to the master demand.
      master1$Skill.Bucket <- train.test$class.label
      
      demand.dump<-rbind(demand.dump,master1[master.length+1:nrow(master1)])
