...
demand.dump <-data.frame(  fread("dump2222.csv", stringsAsFactors = FALSE))
    ...
    for(i in 1:1){
      ...
      prediction <- function(ovrdemand.agg, ext.agg,int.agg, totful.agg){
        ...
        final.results <- data.frame(Year =  as.numeric(ovrdemand.agg$year[nrow(ovrdemand.agg)]), Week = ovrdemand.agg$week[nrow(ovrdemand.agg)]+ c(1:12),  
                                    overall = round(ovr.forecast$Point.Forecast[1:12]))
...
## new function to append the data
append_data<-function(){
  ...
  # setnames(test, old = c("x.name","x1.5.10year","x10g","x10x","x12yr","x14nm","x15th","x178b","x1a","x1p","x2.way","x2008r2","x21cfr","x2d","x2exposur","x3.leg","x300work",
  #                        "x32.bit","x3d","x3p","x3yrs.6yr",
  #                        "x5p","x5th","x5yr","x64.bit","x802.11d"),
  #          new = c("X.name","X1.5.10year","X10g","X10x","X12yr","X14nm","X15th","X178b","X1a","X1p","X2.way","X2008r2","X21cfr","X2d","X2exposur","X3.leg","X300work",
  #                  "X32.bit","X3d","X3p","X3yrs.6yr",
  #                  "X5p","X5th","X5yr","X64.bit","X802.11d"))
  ...
  write.csv(dmp2, "dump2_created.csv")
  ...
  demand.dump<-rbind(demand.dump,new.demand_load1)
...
server <- function(input, output, session) {
  ...
  appenddata<-eventReactive(input$file1,{append_data()})
  
 
  
  demand.dump1<-reactive({
