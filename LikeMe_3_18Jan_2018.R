...
demand.dump$quarter<- quarter(ymd(demand.dump$Approval.Date),with_year = FALSE, fiscal_start = 4)
demand.dump$year <- year(ymd(demand.dump$Approval.Date))
demand.dump$month <- month(ymd(demand.dump$Approval.Date))
maxdate <- max(ymd(demand.dump$Approval.Date))
maxdate_req <- max(dmy(demand.dump$Req.Date))
...
server <- function(input, output, session) {
  ...
  output$contents <- renderTable({
    ...
    req(input$file1)
    print(1)
    #df <- read.csv(fread("dump2.csv", stringsAsFactors = FALSE))
