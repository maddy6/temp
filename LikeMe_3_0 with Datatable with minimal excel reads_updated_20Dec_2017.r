...
likeme <- function(skill1, job1, exp1, stype1, sk.ill, num1,clack, functional, systems, jobboard1){    
  ...
  if(jobboard1=="no"){
    if(sk.ill == "I have already entered the skills"){
      ...
      new_requirement <- data.frame(V1 = nrow(candidates)+1,Mobile.Number = 9999999999,Email = "",Profile = job1, Education = "",Skills = skill1, TProfile = "",Years.Exp=exp1 ,requirement = "")
...
cust.forecast <- function(a,b,c,d,country){
  cust.forecast <- Sys.time()
  
  #Selecting data based on the region selected.
  demand <- subset(demand.dump, toupper(demand.dump$country)==toupper(country))
...
combopred <- function(a,b,c,d,country){    
  demand <- subset(demand.dump, toupper(demand.dump$country)==toupper(country))
...
forecaster <- function(skill.input, country,d){
...
maptable <- function(a,b,c,d,country){
  demand <- subset(demand.dump, toupper(demand.dump$country)==toupper(country))
...
##############################################Skill Vs Customer#########################################
custskill1 <-function(c, d, e,f,country){
  #setwd("D:/HCL/Likeme/Demand")
  
  #custmaster <- data.frame( fread("Demand/demand.csv", header = TRUE, stringsAsFactors = FALSE))
  #custmaster<-demand
  custmaster <- subset(demand.dump, toupper(demand.dump$country)==toupper(country))
...
###############################################Dashboard tabs#####################################
tabs <- function(f,g,h,country){
  #setwd("D:/HCL/LikeMe/Demand")
  
  #master <- data.frame( fread("Demand/demand.csv", header = TRUE, stringsAsFactors = FALSE))
  # master<-demand
  master <- subset(demand.dump, toupper(demand.dump$country)==toupper(country))
...
Popular <- function(country,customer, n, buky, expe, band, quat){
  ...
  if (country!=""){
    B<-which(toupper(demand.dump$country) == toupper(country))}
...
fulfillment.customer <- function(skill,country,d){
  #setwd("D:/HCL/LikeMe/Demand")
...
fulfillment.location <-  function(skill,country,d){
  #setwd("D:/HCL/LikeMe/Demand")
  #master <- data.frame( fread("Demand/dump.csv", stringsAsFactors = FALSE))
  master <- subset(demand.dump, toupper(demand.dump$country)==toupper(country))
...
server <- function(input, output, session) {
  ...
  data1 <- eventReactive(input$cust, {forecaster(input$forecast.ss[1],input$custloc[1],input$forecast.ll[1])})
  data2 <- eventReactive(input$cust, {maps(input$forecast.ss[1],input$forecast.qq[1],input$forecast.yy[1],input$forecast.ll[1],input$custloc[1])})
  data3 <- eventReactive(input$cust, {maptable(input$forecast.ss[1],input$forecast.qq[1],input$forecast.yy[1],input$forecast.ll[1],input$custloc[1])})
  ...
  data7 <- eventReactive(input$cust,{custskill1(input$forecast.ss, input$forecast.yy, input$forecast.qq,input$forecast.ll[1],input$custloc[1])})
  data8 <- eventReactive(input$cust,{tabs(input$forecast.ss, input$forecast.yy, input$forecast.qq,input$custloc[1])})
  ...
  data10 <- eventReactive(input$cust,{fulfillment.customer(input$forecast.ss[1],input$custloc[1],input$forecast.ll[1])})
  data11 <- eventReactive(input$cust,{fulfillment.location(input$forecast.ss[1],input$custloc[1],input$forecast.ll[1])})
  data.popularity <- eventReactive(input$popularity,{popularity(input$poploc,input$dynamic,input$dyna)})
  data.combforecast <- eventReactive(input$cust,{combopred(input$forecast.ss[1],input$forecast.qq[1],input$forecast.yy[1],input$forecast.ll[1],input$custloc[1])})
  data.custforecast <- eventReactive(input$cust,{cust.forecast(input$forecast.ss[1],input$forecast.qq[1],input$forecast.yy[1],input$forecast.ll[1],input$custloc[1])})
  ...
  output$forecast.year <- renderUI({
    selectInput("forecast.yy", "Select Year",
                if(input$forecast.ll=="ALL"){
                  ...
                  choices = unique(subset(demand.dump, tolower(demand.dump$country)==tolower(input$custloc) & tolower(demand.dump$Skill.Bucket)==tolower(input$forecast.ss) & tolower(demand.dump$Joining.Level.2)==tolower(input$forecast.ll))$year)  
                },
                selected = "option3"
    )
  })
  ...
  output$forecast.quarter <- renderUI({
    selectInput("forecast.qq", "Select Quarter",
                if(input$forecast.ll=="ALL"){
                  ...
                  choices = unique(subset(demand.dump, tolower(demand.dump$country)==tolower(input$custloc) & tolower(demand.dump$Skill.Bucket)==tolower(input$forecast.ss) &tolower(demand.dump$Joining.Level.2)==tolower(input$forecast.ll) & demand.dump$year == input$forecast.yy)$quarter) 
                },
                selected = "option3"
    )
  })
